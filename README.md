...
RFID is checked first (2 attempts); on failure, keypad password is checked (3 attempts)
On valid access, servo unlocks the door for a fixed duration, then re-locks
Every access attempt (granted or denied) is logged to EEPROM with timestamp
Access status is broadcast on the CAN bus and pushed to ThingSpeak over Wi-Fi
Setup
Open the project in Keil µVision
Build the project
Flash the generated .hex file using Flash Magic
Power the board and connect peripherals as wired
Author
yachcharesh Trainee ID: v25be8y2 Institute: Vector India
